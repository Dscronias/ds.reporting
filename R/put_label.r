...
put_label <- function(data, var, data_label, label_from, label_to) {
    var_label <- data_label %>% 
        dplyr::filter({{label_from}} == rlang::englue("{{var}}")) %>% 
        dplyr::pull({{label_to}})

    # Check that var exists in data_label$label_from
    assertthat::assert_that(
        length(var_label) > 0,
        msg = glue::glue("{rlang::englue('{{var}}')} does not exist in {rlang::englue('{{label_from}}')} (label_from) column")
    )
