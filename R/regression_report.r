...
regression_report <- function(data, workbook, new_wb = TRUE, worksheet, model_label = "Model", lang = "en", depvar, data_label, label_from, label_to, open_on_finish = TRUE, overwrite_file = TRUE, filename) {
    ...
    footer_label <- switch(
        lang,
        "en" = glue::glue("***: p-value < {pval_stars[1]}; **: p-value < {pval_stars[2]}; *: p-value < {pval_stars[3]}. Estimates in bold are statistically significant (p-value < 0.05)."),
        "fr" = glue::glue("***: p-value < {pval_stars[1]}; **: p-value < {pval_stars[2]}; *: p-value < {pval_stars[3]}. Les estimations en gras sont statistiquement significatives (p-value < 0.05)."),
        "math" = glue::glue("***: p-value < {pval_stars[1]}; **: p-value < {pval_stars[2]}; *: p-value < {pval_stars[3]}. Estimates in bold are statistically significant (p-value < 0.05).")
    )
